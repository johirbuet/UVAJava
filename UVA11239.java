...
public class UVA11239 {
	public static void main(String[] args) throws IOException {
		...
		while ((line = in.readLine()).charAt(0) != '0') // The last test case is followed by a line starting with the
														// digit 0
		{
			if (line.charAt(0) == ('1')) // The input contains several test cases, each one ending with a line that
			...
			{
				for (String key : m.keySet()) {
					if (m.get(key).contains(line) && !key.equals(project)) // if the current userID exists in the set
																			// which is not in the current project
					{
						ban.add(line); // add the userID to the ban list
					}
				}
				m.get(project).add(line); // add the userID (which is not already signed up for a different project) to
											// the project
			}
		}
		out.close();
